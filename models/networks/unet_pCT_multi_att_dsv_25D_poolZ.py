...
import torch
from .utils import UnetConv3, UnetUp25d_CT, UnetGridGatingSignal3, UnetDsv3
import torch.nn.functional as F
from models.networks_other import init_weights
from models.layers.grid_attention_layer import GridAttentionBlock3D


class unet_pCT_multi_att_dsv_25D_poolZ(nn.Module):
    """
    ...
    def __init__(self, feature_scale=4, n_classes=2, is_deconv=True, in_channels=4, input_nz = 5,
                 nonlocal_mode='concatenation', attention_dsample=(2,2,1), is_batchnorm=True):
        super(unet_pCT_multi_att_dsv_25D_poolZ, self).__init__()
        ...
        # downsampling
        self.conv1 = UnetConv3(self.in_channels, filters[0], self.is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0))
        self.maxpool1 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.conv2 = UnetConv3(filters[0], filters[1], self.is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0))
        self.maxpool2 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.conv3 = UnetConv3(filters[1], filters[2], self.is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0))
        self.maxpool3 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.conv4 = UnetConv3(filters[2], filters[3], self.is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0))
        self.maxpool4 = nn.MaxPool3d(kernel_size=(2, 2, 1))

        self.center = UnetConv3(filters[3], filters[4], self.is_batchnorm, kernel_size=(3,3,1), padding_size=(1,1,0))
        ...
        # upsampling
        self.up_concat4 = UnetUp25d_CT(filters[4], filters[3], is_batchnorm)
        self.up_concat3 = UnetUp25d_CT(filters[3], filters[2], is_batchnorm)
        self.up_concat2 = UnetUp25d_CT(filters[2], filters[1], is_batchnorm)
        self.up_concat1 = UnetUp25d_CT(filters[1], filters[0], is_batchnorm)

        # deep supervision
        self.dsv4 = UnetDsv3(in_size=filters[3], out_size=n_classes, scale_factor=(8,8,1))
        self.dsv3 = UnetDsv3(in_size=filters[2], out_size=n_classes, scale_factor=(4,4,1))
        self.dsv2 = UnetDsv3(in_size=filters[1], out_size=n_classes, scale_factor=(2,2,1))
        self.dsv1 = nn.Conv3d(in_channels=filters[0], out_channels=n_classes, kernel_size=1)


        # Pool along z
        self.maxpoolZ = nn.MaxPool3d(kernel_size=(1, 1, input_nz))
